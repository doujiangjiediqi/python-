...
fp = open('./修炼成仙的我只想养成女徒弟.txt', "w", encoding='utf-8')  # 创建小说将要保存的txt文件
for dd in origin_dd_list:  # 对小说的每一个章节进行操作
    ...
    if a.strip() == b.strip():
        i = 1
    next_url = str(medium_tree.xpath('//a[@id="next_url"]/@href')[0].encode('utf-8'),
                   'utf-8')
    while i == 1:
        print("找到了")
        url_final = url_prefix + next_url  # 拼接小说章节下一页的url
        ...
        for pp in p_list_final:  # 对小说各个章节的分行内容进行操作
            ...
            print("success")
        a = str(final_tree.xpath('//a[@id="next_url"]/text()')[0].encode('utf-8'), 'utf-8')
        next_url = str(final_tree.xpath('.//a[@id="next_url"]/@href')[0].encode('utf-8'), 'utf-8')
        if a.strip() == b.strip():
            i = 1
        else:
            i = 0
